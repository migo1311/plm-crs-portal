...
class StudyPlan extends Component
{
    ...
    {
        $this->student_id = $student_id; // Assigning the value passed from Livewire component invocation to $this->student_id
    ...
    public function loadStudentData()
    {
        ...
        if ($study_plan_validation) {
            ...
            $this->yearLevel = $study_plan_validation->year_level;
            $this->studyPlanCodes = $study_plan_validation->study_plan;
            $this->allowedCourseCodes = json_decode($this->studyPlanCodes, true) ?: []; // Ensure it's an array
            $this->allowedCourseCodes = array_map('trim', $this->allowedCourseCodes); // Trim whitespace
